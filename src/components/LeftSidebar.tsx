...
interface LeftSidebarProps {
  ...
  hasErrorCategories: string | null;
  ...
  fetchNewsForCategory: (
    ...
    customLinks: string[] | undefined, // Corrected type
    categories: Category[]
  ) => Promise<void>;
  fetchAllNews: () => void;
  ...
  const renderLoadingStates = () => {
    ...
    if (isRetryingCategories) {
        ...
        return (
            ...
            <button
                onClick={fetchCategories} // Assuming this triggers a reload
    ...
    if (hasErrorCategories) {
       // ... (keep existing error state)
        return (
            ...
            <button
                onClick={() => window.location.reload()} // Simple page refresh
    ...
    if (filteredCategories.length === 0 && filterTerm) {
      return (
        <div className="flex flex-col justify-center items-center h-full px-4 text-center pt-10">
  ...
  return (
    ...
    <div className="w-1/3 bg-gray-50 shadow-lg flex flex-col h-screen max-h-screen"> {/* Use h-screen for full height */}
      ...
      {!isLoadingCategories && !hasErrorCategories && categories.length > 0 && (
        <StatsAndFilterBar
            categories={categories} // Pass the *original* full list for stats
            filterTerm={filterTerm}
            onFilterChange={setFilterTerm} // Pass the setter function
        />
      )}
      ...
      {!isLoadingCategories && !hasErrorCategories && categories.length > 0 && (
        <div className="absolute bottom-0 left-0 w-1/3 p-4 bg-white border-t border-gray-200 shadow-inner flex-shrink-0 z-20"> {/* Added z-index */}
          <button
            className={`w-full bg-indigo-600 text-white py-2.5 rounded-lg hover:bg-indigo-700 transition duration-150 ease-in-out shadow-md flex items-center justify-center gap-2 text-sm font-medium

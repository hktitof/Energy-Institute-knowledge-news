// File: pages/api/preview-optimized.ts
...
type ProxyResponse = {
  ...
  error?: string;
  status: number;
  method?: string; // Indicates which method was used (fetch or puppeteer)
};
...
async function puppeteerRender(targetUrl: string, res: NextApiResponse<ProxyResponse>) {
  let browser = null;
  ...
  try {
    ...
    const processedContent = content.replace(/<head>/i, '<head><base target="_blank">').replace(
      "</head>",
      `<style>
          body { max-width: 100%; overflow-x: hidden; }
          img, video, iframe { max-width: 100%; height: auto; }
    ...
    res.status(500).json({
      error: `Puppeteer rendering failed: ${(error as Error).message}`,
      status: 500,
    });
  } finally {
    if (browser) {

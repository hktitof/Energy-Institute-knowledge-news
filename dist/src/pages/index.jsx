...
export default function NewsAggregator({ isTestMode = false }) {
    // Use our custom hooks to handle different concerns
    const { categories, setCategories, isLoading: isLoadingCategories, deletingCategoryId, toggleCategoryTable, deleteCategory, newCategoryName, setNewCategoryName, isRetrying: isRetryingCategories, retryCount: retryCountCategories, error: hasErrorCategories, updateCategories, } = useCategories({ isTestMode });
    const { loadingSearchTermId, removeSearchTerm } = useSearchTerms(categories, setCategories);
    const { categoriesFetching, setCategoriesFetching, isFetchingAllNewsByButton, categoriesStatus, refFetchNews, fetchNewsForCategory, fetchAllNews, } = useArticleFetching(categories, setCategories);
    const { 
    // selectedCategoryId,
    setSelectedCategoryId, selectedCategoryName, setSelectedCategoryName, activeTab, setActiveTab, } = useCategoryLinks(categories, setCategories);
    ...
    useEffect(() => {
        ...
        if (isFetchingAllNewsByButton) {
            ...
            if (currentlyFetchingIndex !== -1 && totalCategories > 0) {
                ...
                console.log(`Title Calculation useEffect: isFetchingAllNewsByButton is true, but no category has isFetchingNewArticles=true.`);
            }
            else {
    ...
    return (<>
      ...
      <div className="flex h-screen bg-gray-50">
        ...
        <LeftSidebar categories={categories} setCategories={setCategories} isLoadingCategories={isLoadingCategories} isRetryingCategories={isRetryingCategories} retryCountCategories={retryCountCategories} hasErrorCategories={hasErrorCategories} deletingCategoryId={deletingCategoryId} toggleCategoryTable={toggleCategoryTable} deleteCategory={deleteCategory} loadingSearchTermId={loadingSearchTermId} removeSearchTerm={removeSearchTerm} categoriesFetching={categoriesFetching} setCategoriesFetching={setCategoriesFetching} categoriesStatus={categoriesStatus} refFetchNews={refFetchNews} fetchNewsForCategory={(categoryId, customLinks, categories) => {
            fetchNewsForCategory(categoryId, customLinks, categories, setCategories);
            return Promise.resolve();
        }} fetchAllNews={fetchAllNews} isFetchingAllNewsByButton={isFetchingAllNewsByButton} setActiveTab={setActiveTab} setSelectedCategoryName={setSelectedCategoryName} fetchCategories={() => fetchCategories(setCategories)} setSelectedCategoryId={setSelectedCategoryId}/>

        {/* Right content area */}
        <div className="w-2/3 overflow-y-auto">
          {/* Add Category and Search Terms Form */}
          <CategoryManager newCategoryName={newCategoryName} setNewCategoryName={setNewCategoryName} setCategories={setCategories} fetchCategories={fetchCategories} setActiveTab={setActiveTab} activeTab={activeTab} selectedCategoryName={selectedCategoryName || ""} categories={categories} updateCategories={updateCategories}/>

          {/* Articles Tables */}
          {categories.map(category => category.showTable && (<CategoryComponent key={`table-${category.id}`} category={category} categories={categories} setCategories={setCategories}/>))}
        </div>
      </div>
    </>);
}

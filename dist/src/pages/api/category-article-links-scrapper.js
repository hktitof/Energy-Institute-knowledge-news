...
async function extractAndSummarize(url, maxWords = 100) {
    try {
        ...
        while (retries <= maxRetries) {
            try {
                aiResponse = await fetch(`${endpoint}/openai/deployments/${deploymentName}/chat/completions?api-version=2024-02-15-preview`, {
                    method: "POST",
                    headers: {
...
async function processUrlList(urls, concurrencyLimit = 3) {
    if (isDevelopment)
    ...
    for (let i = 0; i < urls.length; i += concurrencyLimit) {
        ...
        const batchResults = await Promise.allSettled(batch.map(async (url, batchIndex) => {
        ...
        if (req.method === "GET" || (req.method === "POST" && req.body.searchUrl)) {
            ...
            if (shouldProcessSummaries && articles.length > 0) {
                ...
                for (let i = 0; i < articlesToProcess.length; i += concurrencyLimit) {
                    ...
                    if (isDevelopment)
                        console.log(`Processing article batch ${Math.floor(i / concurrencyLimit) + 1}/${Math.ceil(articlesToProcess.length / concurrencyLimit)}`);
                    const batchResults = await Promise.allSettled(batch.map(async (article, batchIndex) => {
                        const index = i + batchIndex;
                        if (isDevelopment)

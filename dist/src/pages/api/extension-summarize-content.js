...
function extractAllVisibleText(doc) {
    ...
    function extractTextFromNode(node) {
        ...
        const element = node;
        const hasHiddenAttr = element.hasAttribute("hidden") || // Check for hidden attribute
            element.getAttribute("aria-hidden") === "true" || // Check for aria-hidden
            element.getAttribute("type") === "hidden" || // Check for input type hidden
            element.style?.display === "none" || // Check computed style if possible, else basic style attr
    ...
    const body = doc.body;
    if (!body)
        return ""; // Handle cases where body might not exist
...
export default async function handler(req, res) {
    ...
    const { htmlContent, url, maxWords = 150 } = req.body;
    if (!htmlContent) {
    ...
    try {
        ...
        const aiResponse = await fetch(`${endpoint}/openai/deployments/${deploymentName}/chat/completions?api-version=2024-02-15-preview`, {
            method: "POST",
            headers: {
                ...
                messages: [
                    {
                        ...
                        content: "You are an AI assistant designed to summarize web articles concisely based on provided text content. Focus on key information and main points.",
                    },
                    {
                        role: "user",

...
const ArticleExtractor = () => {
    ...
    const handleExtract = async () => {
        ...
        setPromptText("");
        try {
            const res = await axios.post("/api/extract-article", { url });
            if (res.data.success) {
...
${res.data.textContent}`;
                ...
                setError(res.data.error || "Failed to extract article");
            }
        }
        catch (error) {
            console.error(error);
    ...
    return (<div className="w-full max-w-2xl bg-white rounded-lg shadow-lg p-6 mt-8">
      <h1 className="text-2xl font-bold text-center text-gray-800 mb-6">Article Extractor</h1>

      <div className="mb-4">
        <input type="url" className="w-full border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-blue-500" value={url} onChange={e => setUrl(e.target.value)} placeholder="Enter article URL (e.g., https://www.nature.com/articles/...)"/>
      </div>

      <div className="flex justify-center mb-6">
        <button onClick={handleExtract} disabled={loading} className="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-6 rounded-md transition duration-200 disabled:opacity-50">
      ...
      {extractedContent && (<div className="mt-4 space-y-4">
          <div className="bg-gray-50 p-4 rounded-md border border-gray-200">
            ...
            <div className="flex justify-end">
              <button onClick={() => copyToClipboard(extractedContent.textContent)} className="text-sm bg-blue-100 hover:bg-blue-200 text-blue-700 px-3 py-1 rounded-md transition duration-200">
                Copy Full Content
              </button>
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-md border border-gray-200">
            <div className="flex justify-between items-center mb-2">
              ...
              <button onClick={() => copyToClipboard(promptText)} className="text-sm bg-gray-200 hover:bg-gray-300 px-3 py-1 rounded-md transition duration-200">

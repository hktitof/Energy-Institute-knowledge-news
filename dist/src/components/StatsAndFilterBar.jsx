...
const StatsAndFilterBar = ({ categories, filterTerm, onFilterChange }) => {
    ...
    return (<div className="p-4 border-b border-gray-200 bg-gray-50 sticky top-0 z-10">
      {/* Stats Row */}
      <div className="flex flex-wrap gap-x-4 gap-y-2 items-center mb-4 text-xs text-gray-600">
        ...
        {stats.notArticleCount > 0 && (<div className="flex items-center gap-1" title="Non-Article Content Found">
            <AlertCircle size={14} className="text-yellow-500"/>
            <span className="font-medium">{stats.notArticleCount}</span>
            <span className="hidden sm:inline">Not Articles</span>
          </div>)}
        {stats.errorArticleCount > 0 && (<div className="flex items-center gap-1" title="Articles with Errors">
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Filter categories or search terms..." value={filterTerm} onChange={e => onFilterChange(e.target.value)} className="w-full pl-10 pr-4 py-2 text-sm border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 shadow-sm transition duration-150"/>
        {filterTerm && (<button onClick={() => onFilterChange("")} className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-gray-600" aria-label="Clear filter">
            <XCircle size={16}/>
          </button>)}
      </div>
    </div>);

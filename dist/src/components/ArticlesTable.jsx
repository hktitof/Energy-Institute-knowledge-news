...
import { AlertTriangle, // Added for "NOT AN ARTICLE"
XCircle, // Added for errors
 } from "lucide-react";
const ArticlesTable = ({ categories, category, setCategories }) => {
    ...
    const toggleArticleSelection = ({ categoryId, articleId, isShiftKey = false, }) => {
    ...
    const handleSaveEdit = (categoryId, articleId) => {
        setCategories(categories.map((cat) => {
            if (cat.id === categoryId) {
                return {
                    ...cat,
    ...
    const handleCopyLink = (articleId, link) => {
    ...
    const handleCheckboxChange = (categoryId, articleId, event) => {
    ...
    const toggleSummaryExpansion = (articleId) => {
    ...
    return (<>
      <tbody className="bg-white divide-y divide-gray-200">
        ...
        category.articles.map((article, index) => {
            const articleState = getArticleState(article);
            const isSpecialState = articleState === "not_an_article" || articleState === "error";
            return (<tr key={article.id} className={`
                ...
                <td className="px-4 py-3 align-top w-12">
                  <input type="checkbox" checked={article.selected || false} onChange={e => handleCheckboxChange(category.id, article.id, e)} className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"/>
                </td>
                ...
                {articleState === "not_an_article" && (<td colSpan={2} className="px-6 py-4 align-middle text-sm text-yellow-800 bg-yellow-50/70">
                ...
                {articleState === "error" && (<td colSpan={2} className="px-6 py-4 align-middle text-sm text-red-800 bg-red-50/70">
                    <div className="flex items-center gap-3">
                ...
                {articleState === "normal" && (<>
                    ...
                    <td className="px-6 py-4 align-top max-w-xs">
                      {" "}
                      {/* Added max-w-xs */}
                      {editingArticle === article.id ? (<input type="text" value={editTitle} onChange={e => setEditTitle(e.target.value)} className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" aria-label="Edit Title"/>) : (<p className="text-sm font-semibold text-gray-800 break-words">
                          {" "}
                    ...
                    <td className="px-6 py-4 align-top">
                      {editingArticle === article.id ? (<textarea value={editSummary} onChange={e => setEditSummary(e.target.value)} className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" rows={3} aria-label="Edit Summary"/>) : (
                    // --- Start modification here ---
                    <div>
                          {" "}
                          {/* Wrap summary and button */}
                          <p className={`text-sm text-gray-600 break-words ${!expandedSummaries[article.id] ? "line-clamp-3" : "" // Apply line-clamp conditionally
                        }`}>
                            {article.summary || <span className="text-gray-400 italic">No Summary Provided</span>}
                          </p>
                          {/* Show button only if there IS a summary */}
                          {article.summary && (<button onClick={() => toggleSummaryExpansion(article.id)} className="mt-1 text-xs text-indigo-600 hover:text-indigo-800 flex items-center hover:cursor-pointer">
                ...
                <>
                        <button onClick={() => handleSaveEdit(category.id, article.id)} className="inline-flex items-center px-2.5 py-1 text-xs font-medium text-white bg-green-600 rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors" title="Save Changes">
                          <Save size={14} className="mr-1"/>
                          Save
                        </button>
                        <button onClick={() => setEditingArticle(null)} className="inline-flex items-center px-2.5 py-1 text-xs font-medium text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 transition-colors" title="Cancel Edit">
                          <X size={14} className="mr-1"/>
                          Cancel
                        </button>
                      </>) : (
                // Edit Button (when not editing)
                <button onClick={() => handleEditClick(article)} disabled={isSpecialState} // Disable edit for special states
                 className={`inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <button onClick={() => handlePreview(article)} className="p-1.5 text-gray-500 hover:text-indigo-600 hover:bg-indigo-100 rounded-full transition-colors" title="Preview" // Changed title slightly as it's not always a summary preview
            >
                        <Eye size={16}/>
                      </button>
                      ...
                      <a href={article.link} target="_blank" rel="noopener noreferrer" className="p-1.5 text-gray-500 hover:text-blue-600 hover:bg-blue-100 rounded-full transition-colors" title="Open Original Link">
                        <ExternalLink size={16}/>
                      </a>

                      {/* Copy Button (Always show, uses article.link) */}
                      <button onClick={() => handleCopyLink(article.id, article.link)} className={`p-1.5 rounded-full transition-colors ${copiedArticleId === article.id
                    ? "text-green-600 bg-green-100"
                    : "text-gray-500 hover:text-purple-600 hover:bg-purple-100"}`} title={copiedArticleId === article.id ? "Link Copied!" : "Copy Link"}>
      ...
      {/* Preview Modal */}
      {previewArticle && (<div className="fixed inset-0 overflow-y-auto z-50 flex items-center justify-center px-3 py-2">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-full h-full flex flex-col">
            ...
            <div className="px-6 py-2 border-b border-gray-200 flex justify-between items-start">
              <div className="space-y-2 flex-1">
                ...
                {previewArticle.link && (<div className="flex items-center text-xs text-gray-500">
                    <span className="inline-block mr-1">Source:</span>
                    <a href={previewArticle.link} target="_blank" rel="noopener noreferrer" className="hover:underline text-blue-600">
                      {(() => {
                    try {
              ...
              <button onClick={handleClosePreview} className="text-gray-400 hover:text-gray-500 focus:outline-none hover:cursor-pointer">
            ...
            <div className="flex-1 overflow-hidden relative bg-gray-100">
              ...
              {/* Loading Overlay */}
              {isLoading && (<div className="absolute inset-0 flex items-center justify-center bg-white bg-opacity-80 z-20">
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-white z-10">
                    <div className="text-center p-6 max-w-lg">
                      ...
                      <a href={previewArticle.link} // Use previewArticle here
             target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        Open in New Tab
                      </a>
                    </div>
                  </div>)}
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-start overflow-auto p-4 bg-white z-10">
                    ...
                    <Image src={previewScreenshot} // Assumes this is a base64 data URL or external URL
             alt={`Screenshot of ${previewArticle?.title || "article"}`} className="max-w-full h-auto border border-gray-200 rounded shadow-sm" // Use h-auto
            ...
            />
              ...
              <iframe id="preview-iframe" title={previewArticle?.title || "Article Preview"} // Use optional chaining
         className={`w-full h-full border-0 transition-opacity duration-300 ${isLoading || previewError || useScreenshot ? "opacity-0" : "opacity-100" // Control visibility via opacity
            }`} sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-modals allow-popups-to-escape-sandbox"/>
            </div>

            {/* Modal Footer */}
            <div className="p-4 border-t border-gray-200 flex justify-between">
              <a href={previewArticle.link} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 flex items-center">
                <ExternalLink size={16} className="mr-2"/>
                Open in New Tab
              </a>
              <button onClick={handleClosePreview} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                Close
              </button>
            </div>
          </div>

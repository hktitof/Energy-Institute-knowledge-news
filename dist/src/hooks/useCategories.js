...
export const refetchCategories = async (setCategories) => {
    try {
...
export const useCategories = ({ isTestMode = false, maxRetries = 5, retryDelay = 3000 }) => {
    ...
    const loadCategories = useCallback(async (retry = 0) => {
        ...
        try {
            ...
            if (axios.isAxiosError(error) && error.response?.status === 503) {
                ...
                if (retry < maxRetries) {
                    // Wait and retry
                    toast.info(`Database is resuming. Retrying in ${retryDelay / 1000} seconds... (${retry + 1}/${maxRetries})`);
    ...
    const toggleCategoryTable = (categoryId) => {
    ...
    const updateArticleInState = useCallback(({ categoryId, articleId, newDetails }) => {
        console.log(`[NextApp] Received request to update article: categoryId=${categoryId}, articleId=${articleId}`, newDetails);
    ...
    useEffect(() => {
        ...
        if (window[NEXT_APP_STATE_ACCESSOR]) {
            console.log(`[NextApp] typeof window[${NEXT_APP_STATE_ACCESSOR}].getState:`, typeof window[NEXT_APP_STATE_ACCESSOR].getState);
        }
        else {
            console.warn(`[NextApp] window[${NEXT_APP_STATE_ACCESSOR}] is still undefined immediately after assignment attempt!`);
        }
        // --- END OF ADDED LOG ---
        return () => {

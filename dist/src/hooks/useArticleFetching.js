...
export const useArticleFetching = (categories, setCategories) => {
    // this useState is used to track the selected fetching Category and will be used when fetching multiple categories
    const [categoriesFetching, setCategoriesFetching] = useState(null);
    const [isFetchingAllNewsByButton, setIsFetchingAllNewsByButton] = useState(false);
    const [categoriesStatus, setCategoriesStatus] = useState([]);
    const refFetchNews = useRef(null);
    // Initialize categoriesStatus based on categories
    useEffect(() => {
        const categoriesBeingFetched = categories.filter(category => category.articles.some(article => article.title === ""));
        setCategoriesStatus(categoriesBeingFetched.map(category => ({
    ...
    const extractSummary = (rawSummary) => {
    ...
    const fetchNewsForCategory = async (categoryId, customLinks = [], categories = [], setCategories) => {
        ...
        try {
            ...
            const searchUrl = category.searchTerms.length > 0
                ? `https://www.google.co.uk/search?q=${encodeURIComponent(category.searchTerms.join(" OR "))}&tbm=nws&tbs=qdr:w`
                : "";
            let linksToProcess = [...customLinks];
        ...
        try {
            ...
            for (let i = 0; i < updatedCategories.length; i++) {
                ...
                try {
                    ...
                    updatedCategories = updatedCategories.map(cat => cat.id === category.id ? { ...cat, isFetchingNewArticles: false } : cat);
                    setCategories([...updatedCategories]);
    ...
    const refetchArticles = useCallback(async ({ categoryId }) => {
        ...
        if (articlesToFetch.length === 0) {
            ...
            const categoriesUpdates = categories.map(cat => cat.id === categoryId ? { ...cat, isFetchingNewArticles: false } : cat);
            const statusUpdates = categoriesStatus.map(status => status.categoryId === categoryId
        ...
        try {
            ...
            const categoriesUpdates = categories.map(cat => cat.id === categoryId ? { ...cat, articles: updatedArticles, isFetchingNewArticles: false } : cat);
            const statusUpdates = categoriesStatus.map(status => status.categoryId === categoryId
                ? { ...status, isFetchingArticles: false, isFetchedAllArticles: true }
                : status);
            setCategories(categoriesUpdates);
            ...
            setCategoriesStatus(prevStatus => {
                return prevStatus.map(status => status.categoryId === categoryId
                    ? { ...status, isFetchingArticles: false, isFetchedAllArticles: true }
                    : status);
            });
    ...
    useEffect(() => {
        const shouldFetchArticles = categoriesStatus.some(status => status.isFetchingArticles && !status.isFetchedAllArticles);
        ...
        const fetchArticles = async () => {
            ...
            const categoryStatusToFetch = categoriesStatus.find(status => status.isFetchingArticles && !status.isFetchedAllArticles);
